...
def readSim(simulated_task_location):
    try:
        ...
        for simRepeat in range(1,21):
            ...
            if simRepeat < 10:
                file = simulated_task_location[0:-6] + '_' + str(simRepeat) + ".csv"
            else:
                file = simulated_task_location[0:-6]+ '_' + str(simRepeat) + ".csv"
            print(file)
            #tmp = np.genfromtxt(Path(file), comments="#", delimiter=",") #Open file and read into numpy array
            tmp = pd.read_csv(Path(file),skiprows=[i for i in range(10)]).values[:6236,:]
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            simulated_task_location = r"E:\Server_mantain\robotic_arm\Dataset\Task1\RawData\task1_test_1.csv"
        elif opt in ("-d", "--dfile"):
            #groundtruth_location = arg
            groundtruth_location = r"E:\Server_mantain\robotic_arm\Dataset\Task1\Task1_mean10Hz.csv"
